...
# Configure logging for the background VIP bot
logging.basicConfig(
...
logging.getLogger("telegram_vip_bot").setLevel(logging.INFO)

from src.agent import DigitalTwinAgent
import vip_bot

def run_vip_bot_in_background():
    loop = asyncio.new_event_loop()
    asyncio.set_event_loop(loop)
    try:
        loop.run_until_complete(vip_bot.start_bot())
    except Exception as e:
        print(f"\n❌ Error running VIP Bot in background: {e}")
    finally:
        loop.close()
...
def main():
    ...
    # Start VIP Payment Bot in a background thread
    print("🚀 Memulai VIP Payment Bot di background...")
    vip_thread = threading.Thread(target=run_vip_bot_in_background, daemon=True)
    vip_thread.start()
