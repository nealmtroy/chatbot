...
async def polling_loop(client, config, store):
    LOGGER.info("Starting VIP Payment Polling Loop...")
    while True:
        try:
            ...
            for payment in pending:
                expires_at = parse_iso_datetime(payment.get("qris_expires"))
                if payment["status"] == "pending" and expires_at and expires_at <= dt.datetime.now(dt.timezone.utc):
...
async def broadcast_loop(client, config, store):
    LOGGER.info("Starting VIP Broadcast Loop...")
    while True:
        try:
            ...
            broadcast_day_start = now.replace(hour=0, minute=0, second=0, microsecond=0).astimezone(dt.timezone.utc).isoformat()
