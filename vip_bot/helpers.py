...
def user_link(row):
    name = html.escape(row.get("full_name") or str(row.get("user_id")))
    return f'<a href="tg://user?id={row.get("user_id")}">{name}</a>'
...
def create_qris_with_retries_sync(config, user, amount=None, note_prefix="VIP", attempts=3):
    ...
    for attempt in range(1, attempts + 1):
        try:
            ...
            uid = getattr(user, "id", user)
            LOGGER.warning("SociaBuzz timed out while creating %s QRIS for user %s, retrying in %.1fs (%s/%s): %s", note_prefix, uid, delay, attempt, attempts, exc)
            time.sleep(delay)
    raise last_exc
...
async def delete_qris_message(client, payment):
    ...
    try:
        ...
        LOGGER.info("QRIS message %s in chat %s for invoice %s not deletable: %s", message_id, chat_id, invoice_id, exc)


async def create_invite_link(client, config, store, payment):
    package_code = (payment.get("package_code") or "").lower()
    if "vcs" in package_code:
        return "VCS_PAID", None
    ...
    expires_at = dt.datetime.now(dt.timezone.utc) + dt.timedelta(hours=invite_hours)
