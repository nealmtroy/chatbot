...
class PaymentStore:
    ...
    def get_payment_bot(self, token_or_username):
        if not self.client or not token_or_username:
            return None
    ...
    def delete_payment_bot(self, token_or_username):
        ...
        query = (
            self.client.table(self.payment_bots_table)
            .update({"active": False, "updated_at": utc_now_iso()})
            .or_(f"bot_token.eq.{target},bot_username.eq.{target}")
        )
    ...
    def create_referral_if_absent(self, referrer, invited_user):
        ...
        data = {
            ...
            "invited_username": getattr(invited_user, "username", "") or "",
            "invited_full_name": display_name(invited_user) if hasattr(invited_user, "first_name") else f"User_{invited_id}",
            "status": "pending",
            "created_at": utc_now_iso(),
            "updated_at": utc_now_iso(),
        }
    ...
    def create_withdrawal(self, user, amount, details):
        ...
        response = self._execute(
            self.client.rpc(
                ...
                {
                    "p_user_id": user_id,
                    "p_username": getattr(user, "username", "") or "",
                    "p_full_name": display_name(user) if hasattr(user, "first_name") else f"User_{user_id}",
                    "p_amount": int(amount),
