...
def qris_caption(package, inv_id, checkout_amount, final_amount, expires):
    ...
    package_name = html.escape(package.get("name") or "VIP")
...
def custom_qris_caption(inv_id, checkout_amount, final_amount, expires, user):
    ...
    detail_lines = [
        f"<b>Kode Pesanan</b>: <code>{public_invoice}</code>",
        f"<b>Requester</b>: {telegram_user_link(user)} (<code>{getattr(user, 'id', user)}</code>)",
...
def package_buttons(config, store, bot_username=None):
    try:
        packages = store.list_packages(bot_username=bot_username)
    except Exception:
        packages = []
    if not packages:
